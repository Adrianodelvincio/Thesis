...
\begin{itemize}
\item $(a = - 802.1  \pm 1.1) \cdot 10^{6} \, \frac{\SI{}{\milli \volt}}{[arb.unit]}$
\item $b = 282.51 \pm 0.06 \, [arb. unit]$
\item $c = -6.147 \pm 0.009 \, \SI{}{\milli \volt} $
\end{itemize}
...
\begin{equation}
\chi^{2}_{n-1} = \sum_{i = 1}^{n} \dfrac{(Obs_{i} - Exp_{i})^{2}}{Exp_{i}}
\end{equation}
...
We will discuss later, when we will introduce the fit results, whether our assumption reflects the reality. We assume that the only false asymmetry that has an effect is $\delta I$: $\overline{\delta I} = -1.23 \, ppm$, and we will subtract that to the final result:
