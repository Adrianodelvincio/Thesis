...
\textbf{Explain the test of the two detectors in the lab, how we select the threshold, the correlation of the pmts and coincidence to select the threshold. Mention also that we observed two knees in the plot of counts vs. attenuation.}\\
...
\textbf{Here it's important to show the plots I made during the beam time. I have to mention the Leo tecniques for the correct interpretation of counts vs attenuation.}\\
...
We can check the idea behind this, visualizing the pmt count in a different way. In this plot (\ref{fig:Spectra}) there is a discrete differentiation of the data showed in (\ref{fig:FitAtt}), essentially divide neighboring counts by distance in attenuation. This plot represents roughly the signal spectra that of our pmts. 
