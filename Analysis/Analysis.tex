...
After this first general data selection, we focus our attention on the correlated-difference data, which are important to estimate the contributions of the false-asymmetries. 

\begin{align*}
\delta x =  \frac{(x_{up,1} + x_{up,2})}{2} - \frac{(x_{down,1} + x_{down,2})}{2}
\end{align*}

An histogram is produced for each beam parameter correlated-difference. These plots are useful to study the beam-stabilization, which is an essential feature of the experiment to decrease the contributions of the false asymmetries to the total asymmetry.
We expect, if the stabilization of the beam is working, that all the correlated differences are distributed around zero, which implies that there is no systematic difference when the beam is polarized in one direction respect to the other. 
...
\begin{table} 
\centering
\caption{Beam parameters Histograms}
...
these results are encouraging, the stabilization has the consequence that only small differences are observed within an event for each monitor. So, unless the values for the false asymmetries are very large, the effect of the beam variation on the reconstructed asymmetry should be small compared to the physical effect $A_{n}$.

After removing the outliers, we mention an issue which took place on the night of the 2nd December; MAMI operators that were controlling the first stage of the accelerator, because of a wrong setting, switched from polarized beam to non- polarized. This issue introduce a big systematic error that is potentially able to decrease the experimental \transv. We find out this problem only the next day, so we had not en evidence of how many hours of beam were lost. Unfortunately,  because this happened during the night, nobody could save the polarization measurement of the beam and identify the runs affected by this problem. This is a serious trouble, if we are not able to identify the runs that share this problem, out measurements are affected by a bias that is not possible to disentangle from any other systematic effects related to the electronics system used, therefore also the electronic testing is not possible. 
...
Even if those data are not useful in reconstructing the final asymmetry, they can be used to check the presence of a small offset in the data:
...
For a better visualization of the data, especially to observe the dependence of the asymmetry on the Beam parameters measured, it is useful to take the average asymmetry at regular intervals. From the raw plots of the asymmetries (see below, it is clear that the statistical error is too hight to identify a linear structure in the dependence of the asymmetry. 


In all the figures the asymmetries are given in [ppm] units. The data are selected removing the correlated-difference that are $5\sigma$ far from the mean values, that we have reported in the table (\ref{Tab:parametri}) 
